...
class Blocks {
    ...
    blocklyListen(e) {
        ...
        switch (e.type) {
            ...
            case "var_rename":
                if (
                    editingTarget &&
                    editingTarget.variables.hasOwnProperty(e.varId)
                ) {
            ...
            case "comment_move":
                ...
                if (this.runtime.getEditingTarget()) {
                    ...
                    if (
                        currTarget &&
                        !currTarget.comments.hasOwnProperty(e.commentId)
                    ) {
                        log.warn(
                            `Cannot change comment with id ${e.commentId} because it does not exist.`
                        );
        ...
        switch (args.element) {
            ...
            case "checkbox": {
                ...
                if (block.opcode === "data_variable") {
                    isSpriteLocalVariable = !this.runtime.getTargetForStage()
                        .variables[block.fields.VARIABLE.id];
                } else if (block.opcode === "data_listcontents") {
                    isSpriteLocalVariable = !this.runtime.getTargetForStage()
                        .variables[block.fields.LIST.id];
                }

                const isSpriteSpecific =
                    isSpriteLocalVariable ||
                    (this.runtime.monitorBlockInfo.hasOwnProperty(
                        block.opcode
                    ) &&
    ...
    moveBlock(e) {
        ...
        console.log("moveBlock - workSpace running .......");
    ...
    updateBlocksAfterVarRename(varId, newName) {
        console.log("WORKSPACE updateBlocksAfterVarRename 954784578147");
    ...
    updateTargetSpecificBlocks(isStage) {
        console.log("updateTargetSpecificBlocks BLOCKS 9578441121258s");
        const blocks = this._blocks;
        for (const blockId in blocks) {
            if (
                isStage &&
